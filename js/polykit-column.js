...
function polykit_setup_native_inline_actions() {
  ...
  jQuery($gp.editor.table).onFirst(
    "click",
    "td.inline-actions .inline-action",
    polykit_inline_action_loading,
  );
}
...
function polykit_inline_action_loading(e) {
  const button = ("BUTTON" === e.target.nodeName)
    ? e.target
...
function polykit_add_column_buttons(tr_preview) {
  ...
  if (tr_preview.nextElementSibling != null) {
    tr_preview.nextElementSibling.querySelectorAll(
      ".status-actions button.approve,.status-actions button.reject,.status-actions button.fuzzy",
    ).forEach((button) => {
      ...
      clone_button.addEventListener("click", (ev) => {
        const clicked_button = ("BUTTON" === ev.target.parentElement.nodeName)
