...
function gd_quicklinks( current_editor = '.editor' ) {
	...
	const gd_quicklinks_copy = gd_create_element(
		...
		{
			'type':       'button',
			'class':      `gd_quicklinks_copy with-tooltip ${( polykit_quicklinks_copy_state ) ? 'active' : 'inactive'}`,
	...
	const gd_quicklinks_separator = gd_create_element(
		'span',
		{
			'class':       `gd_quicklinks_plus dashicons ${( polykit_quicklinks_copy_state ) ? 'dashicons-plus' : 'separator'}`,
			'aria-hidden': 'true',
		},
	);
