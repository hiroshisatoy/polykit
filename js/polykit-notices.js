...
function polykit_selected_count() {
	...
	if ( checked_count > 0 ) {
		...
		if ( current_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_current', current_selectedcount ) );
		}
		if ( waiting_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_waiting', waiting_selectedcount ) );
		}
		if ( fuzzy_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_fuzzy', fuzzy_selectedcount ) );
		}
		if ( old_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_old', old_selectedcount ) );
		}
		if ( rejected_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_rejected', rejected_selectedcount ) );
		}
		if ( untranslated_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_untranslated', untranslated_selectedcount ) );
		}
		if ( warnings_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_warnings', warnings_selectedcount ) );
		}
		if ( polykit_selectedcount > 0 ) {
			selected_strings_text.push( polykit_t( 'selected_glossary', polykit_selectedcount ) );
		}
		jQuery( '#polykit-notices-container' ).append( `<div id="polykit-checked-count" class="notice">${polykit_t( 'notice_rows_selected', checked_count )}</div>` );
		if ( Array.isArray( selected_strings_text ) && selected_strings_text.length > 0 ) {
			jQuery( '#polykit-checked-count' ).append( ` (${selected_strings_text.join( '、' )}。)` );
		}
	}
}
...
jQuery( document ).ajaxSuccess( ( event, xhr, settings ) => {
	if ( typeof $gp_editor_options !== 'undefined' && settings.url === $gp_editor_options.set_status_url ) {
		...
		case 'current':
			...
			approved_count++;
			jQuery( '#polykit-notices-container' ).append( `<div id="polykit-approved-count" class="notice approved">${polykit_t( 'notice_approved', approved_count )}</div>` );
			break;
		case 'rejected':
			jQuery( '#polykit-rejected-count' ).remove();
			rejected_count++;
			jQuery( '#polykit-notices-container' ).append( `<div id="polykit-rejected-count" class="notice rejected">${polykit_t( 'notice_rejected', rejected_count )}</div>` );
			break;
		case 'fuzzy':
			jQuery( '#polykit-fuzzied-count' ).remove();
			fuzzied_count++;
			jQuery( '#polykit-notices-container' ).append( `<div id="polykit-fuzzied-count" class="notice fuzzied">${polykit_t( 'notice_fuzzied', fuzzied_count )}</div>` );
			break;
		}
		...
		submitted_count++;
		jQuery( '#polykit-notices-container' ).append( `<div id="polykit-submitted-count" class="notice submitted">${polykit_t( 'notice_submitted', submitted_count )}</div>` );
	}
} );
