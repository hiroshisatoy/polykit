const jsScripts = [ 'jquery.bind-first', 'dompurify', 'keymaster', 'polykit-locales', 'polykit-functions', 'polykit-i18n', 'polykit-gp-l10n', 'polykit-settings', 'polykit-hotkey', 'polykit-validation', 'polykit-locale-validation', 'polykit-column', 'polykit-meta', 'polykit-bulk', 'polykit-notices', 'polykit-consistency', 'polykit' ];
		...
		chrome.runtime.sendMessage(
			...
			( response ) => {
				...
				if ( 'undefined' !== response &&
					...
					polykit_extension_storage.currentVersion !== response['currentVersion'] ) {
					let data = {};
					data = response;
					const lastChange = changelogData.match( /(\* [\s\S]*?)(?=#)/ );
					data['changelog'] = ( null !== lastChange ) ? lastChange[1] : '';
					localStorage.setItem( 'polykit_extension_status', JSON.stringify( data ) );
				}
			},
		);
	} )
