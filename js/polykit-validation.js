    ...
    translations.each((i, translation) => {
      ...
      jQuery(".glossary-word", originals[original_index]).each(
        (j, glossary_element) => {
          ...
          const glossary_word_occurrence = glossary_words.filter((word) =>
            word.toLowerCase() === glossary_word
          ).length;
          ...
          jQuery(glossary_word_translations).each((index) => {
            ...
            if (translation_word_occurrence < glossary_word_occurrence) {
              ...
              const diff = glossary_word_occurrence -
                translation_word_occurrence;
          ...
          if (reset !== "") {
            ...
            const form = translations.length > 1
              ...
              : "";
            let is_within_URL = polykit_check_for_URL(
              glossary_word,
              translatedText,
            );
...
/**
 * 未無視の GlotPress 警告数（オプションで除外する項目を反映）。
 *
 * @param {string} selector
 * @returns {number}
 */
function polykit_count_gp_warnings(selector) {
  let count = 0;
...
function polykit_validate(e, selector) {
  ...
  if ("undefined" === typeof jQuery(selector).data("discard")) {
    ...
    howmany += polykit_count_gp_warnings(selector);
    ...
    if ("undefined" === typeof newtext || "" === newtext) {
      jQuery(".textareas", selector).prepend(
        polykit_get_warning(polykit_t("empty_translation"), discard),
      );
      howmany++;
      return;
    }
    ...
    if (hellipseoriginaltext) {
      if (!polykit_get_setting("no_final_dot")) {
        if (
          "..." === jQuery("textarea", selector).val().slice(-3) ||
          (lastcharnewtext !== ";" && lastcharnewtext !== "." &&
            lastcharnewtext !== "…")
        ) {
          jQuery(".textareas", selector).prepend(
            polykit_get_warning(polykit_t("missing_ellipsis"), discard),
          );
          howmany++;
        }
      }
      ...
      if (!polykit_get_setting("no_final_other_dots")) {
        if (
          ...
          jQuery(".textareas", selector).prepend(
            polykit_get_warning(polykit_t("missing_end_punct"), discard),
          );
          howmany++;
        }
      }
    ...
    if (!polykit_get_setting("no_initial_space")) {
      if (
        ...
        jQuery(".textareas", selector).prepend(
          polykit_get_warning(polykit_t("missing_initial_space"), discard),
        );
        howmany++;
      }
    }
    if (!polykit_get_setting("no_trailing_space")) {
      if (
        ...
        jQuery(".textareas", selector).prepend(
          polykit_get_warning(polykit_t("missing_trailing_space"), discard),
        );
        howmany++;
      }
    }
    ...
    howmany += polykit_validate_locale(
      e,
      selector,
      originaltext,
      newtext,
...
function polykit_get_discard_link(selector) {
  return ` <a href="#" class="discard-polykit" data-row="${
    jQuery(selector).attr("row")
  }">${polykit_t("discard")}</a>`;
}
