let polykit_quicklinks_copy_state =
  "true" === localStorage.getItem("polykit_quicklinks_copy_state");
...
function polykit_quicklinks(current_editor = ".editor") {
  ...
  const polykit_quicklinks_copy = polykit_create_element(
    ...
    {
      "type": "button",
      "class": `polykit-quicklinks-copy with-tooltip ${
        polykit_quicklinks_copy_state ? "active" : "inactive"
      }`,
  ...
  const polykit_quicklinks_permalink = polykit_create_element("button", {
    "class":
      "polykit-quicklinks-item polykit-quicklinks-permalink with-tooltip",
  ...
  const polykit_quicklinks_consistency = polykit_create_element("button", {
    "class":
      "polykit-quicklinks-item polykit-quicklinks-consistency with-tooltip",
  ...
  const polykit_quicklinks_discussion = polykit_create_element("button", {
    "class":
      "polykit-quicklinks-item polykit-quicklinks-discussion with-tooltip",
  ...
  document.querySelectorAll(`${current_editor}`).forEach((editor) => {
    ...
    editor.querySelector(".polykit-quicklinks-permalink").dataset.quicklink =
      editor_menu[0].href;
    editor_menu[1].href += "&historypage";
    editor.querySelector(".polykit-quicklinks-history").dataset.quicklink =
      editor_menu[1].href;
    editor.querySelector(".polykit-quicklinks-consistency").dataset.quicklink =
      `${editor_menu[2].href}&consistencypage`;
    editor.querySelector(".polykit-quicklinks-discussion").dataset.quicklink =
      editor_menu[3].href;
  });
...
async function polykit_do_consistency(el) {
  ...
  this_panel_content.querySelectorAll("textarea").forEach(
    (translation_form) => {
      current_string.translated_texts[current_string.translated_texts.length] =
...
function polykit_consistency_get_alternative_count(
  ...
  if (1 === consistency_count) {
    const unique_alternative_count =
      consistency_page.querySelectorAll("tr").length - 2;
    return [
      ` (${unique_alternative_count} time${
        (unique_alternative_count > 1) ? "s" : ""
      })`,
    ];
  }
...
function polykit_consistency_add_alternative(alternative, current_string) {
  ...
  alternative.forms_text.forEach((form_text, form_text_i) => {
    const is_this_one =
      (form_text === current_string.translated_texts[form_text_i])
    ...
    const polykit_consistency_item_raw = polykit_create_element("span", {
      "class":
        `translation-suggestion__translation-raw consistency_alternative__${alternative.i}_${form_text_i}`,
      "aria-hidden": "true",
    }, form_text);
...
function polykit_notranslate(current_editor = ".editor") {
  ...
  notranslate_header.append(
    polykit_create_element("button", {
      "type": "button",
      "class": "polykit-notranslate-copy-all",
    }, polykit_t("notranslate_copy_all")),
  );
  document.querySelectorAll(
    `${current_editor.replace("editor", "preview")} .original`,
  ).forEach((original_preview) => {
    const editor = original_preview.parentNode.nextElementSibling;
    const notranslate = polykit_create_element("div", {
      "class": "polykit-notranslate",
    });
    const notranslate_fragment = document.createDocumentFragment();
    const original_preview_forms = original_preview.querySelectorAll(
      ".original-text",
    );
    let has_notranslate = false;

    original_preview_forms[0].parentNode.querySelectorAll(
      ".original-text > .notranslate",
    ).forEach((item) => {
      const notranslate_item = document.createElement("a");
      notranslate_item.setAttribute(
        "title",
        polykit_t("notranslate_insert_hint"),
      );
    ...
    if (has_notranslate) {
      ...
      const suggestion_wrapper_el = editor.querySelector(
        ".suggestions-wrapper",
      );
      suggestion_wrapper_el && suggestion_wrapper_el.prepend(notranslate);
    }
  });
  document.querySelectorAll(".editor .notranslate").forEach((el) => {
    el.setAttribute("title", polykit_t("notranslate_insert_hint"));
  });
...
function polykit_notranslate_events(current_editor) {
  polykit_add_evt_listener(
    "click",
    `${current_editor} .translation-form-list li button`,
    (ev) => {
      const textareas =
        ev.currentTarget.closest(".translation-wrapper").querySelectorAll(
          ".textareas",
        )[ev.currentTarget.dataset.pluralIndex];
  ...
  polykit_add_evt_listener(
    "focusin",
    `${current_editor} textarea`,
    polykit_notranslate_update,
  );
  polykit_add_evt_listener(
    "keyup",
    `${current_editor} textarea`,
  ...
  polykit_add_evt_listener(
    "click",
    `${current_editor} .polykit-notranslate a, ${current_editor} .notranslate`,
    (ev) => {
      polykit_notranslate_insertText(
        ev.currentTarget.closest(".editor-panel__left").querySelector(
          ".textareas.active textarea",
        ),
  ...
  polykit_add_evt_listener(
    "click",
    `${current_editor} .polykit-notranslate-copy-all`,
    (ev) => {
      let all_notranslate = "";
      ...
      polykit_notranslate_insertText(
        notranslate_div.closest(".editor-panel__left").querySelector(
          ".textareas.active textarea",
        ),
        all_notranslate,
      );
  ...
  const unique_editor = document.querySelector(
    '.editor[style*="display: table-row;"] textarea',
  );
  if (unique_editor) {
    unique_editor.blur();
    unique_editor.focus();
...
function polykit_notranslate_insertText(el, text) {
  ...
  el.value = el.value.slice(0, selection.start) + text +
    el.value.slice(selection.end);
  el.focus();
...
function polykit_notranslate_update(ev) {
  ...
  ev.currentTarget.closest(".editor-panel__left").querySelectorAll(
    ".polykit-notranslate a",
  ).forEach((notranslate_item) => {
