...
function polykit_quicklinks( current_editor = '.editor' ) {
	const polykit_quicklinks_output = polykit_create_element( 'span', { 'class': 'polykit-quicklinks' } );
	...
	polykit_quicklinks_copy.append(
		polykit_create_element( 'span', { 'class': 'screen-reader-text' }, polykit_t( 'ql_copy_toggle' ) ),
		polykit_create_element( 'span', { 'class': 'dashicons dashicons-clipboard', 'aria-hidden': 'true' } ),
	);
	const polykit_quicklinks_separator = polykit_create_element(
		'span',
		{
			'class':       `polykit-quicklinks-plus dashicons ${( polykit_quicklinks_copy_state ) ? 'dashicons-plus' : 'separator'}`,
			'aria-hidden': 'true',
		},
	);
	const polykit_quicklinks_permalink = polykit_create_element( 'button', { 'class': 'polykit-quicklinks-item polykit-quicklinks-permalink with-tooltip', 'aria-label': polykit_t( 'ql_permalink' ) } );
	polykit_quicklinks_permalink.append(
		polykit_create_element( 'span', { 'class': 'screen-reader-text' }, polykit_t( 'ql_permalink' ) ),
		polykit_create_element( 'span', { 'class': 'dashicons dashicons-admin-links', 'aria-hidden': 'true' } ),
	);

	const polykit_quicklinks_history = polykit_create_element( 'button', { 'class': 'polykit-quicklinks-item polykit-quicklinks-history with-tooltip', 'aria-label': polykit_t( 'ql_history' ) } );
	polykit_quicklinks_history.append(
		polykit_create_element( 'span', { 'class': 'screen-reader-text' }, polykit_t( 'ql_history' ) ),
		polykit_create_element( 'span', { 'class': 'dashicons dashicons-backup', 'aria-hidden': 'true' } ),
	);

	const polykit_quicklinks_consistency = polykit_create_element( 'button', { 'class': 'polykit-quicklinks-item polykit-quicklinks-consistency with-tooltip', 'aria-label': polykit_t( 'ql_consistency' ) } );
	polykit_quicklinks_consistency.append(
		polykit_create_element( 'span', { 'class': 'screen-reader-text' }, polykit_t( 'ql_consistency' ) ),
		polykit_create_element( 'span', { 'class': 'dashicons dashicons-list-view', 'aria-hidden': 'true' } ),
	);

	const polykit_quicklinks_discussion = polykit_create_element( 'button', { 'class': 'polykit-quicklinks-item polykit-quicklinks-discussion with-tooltip', 'aria-label': polykit_t( 'ql_discussion' ) } );
	polykit_quicklinks_discussion.append(
		polykit_create_element( 'span', { 'class': 'screen-reader-text' }, polykit_t( 'ql_discussion' ) ),
		polykit_create_element( 'span', { 'class': 'dashicons dashicons-format-chat', 'aria-hidden': 'true' } ),
	);
	...
	polykit_add_elements( `${current_editor} .editor-panel__left .panel-header .panel-header-actions`, 'afterBegin', polykit_quicklinks_output );
	...
	document.querySelectorAll( `${current_editor}` ).forEach( ( editor ) => {
		...
		editor.querySelector( '.polykit-quicklinks-consistency' ).dataset.quicklink = `${editor_menu[ 2 ].href}&consistencypage`;
		editor.querySelector( '.polykit-quicklinks-discussion' ).dataset.quicklink = editor_menu[ 3 ].href
	} );

	polykit_add_evt_listener( 'click', `${current_editor} .polykit-quicklinks-copy, ${current_editor} .polykit-quicklinks-plus`, polykit_toggle_quicklinks_copy );
	polykit_add_evt_listener( 'click', `${current_editor} .polykit-quicklinks-item`, polykit_do_quicklinks );
}
...
function polykit_do_quicklinks( event ) {
	if ( polykit_quicklinks_copy_state ) {
		...
		polykit_quicklinks_window = window.open( event.currentTarget.dataset.quicklink, '_blank' );
	}
}
...
function polykit_toggle_quicklinks_copy() {
	...
	localStorage.setItem( 'polykit_quicklinks_copy_state', polykit_quicklinks_copy_state );
}
...
function polykit_consistency( current_editor = '.editor' ) {
	...
	const polykit_consistency_output = polykit_create_element( 'details', { 'class': 'polykit-consistency suggestions__translation-consistency', 'open': 'open' } );
	const polykit_consistency_summary = polykit_create_element( 'summary', { }, polykit_t( 'consistency_title' ) );
	const polykit_consistency_loading = document.querySelector( '.suggestions__loading-indicator' );

	polykit_consistency_output.append( polykit_consistency_summary );
	polykit_consistency_loading && polykit_consistency_output.append( polykit_consistency_loading );
	polykit_add_elements( `${current_editor} .editor-panel__left .suggestions-wrapper .suggestions__translation-memory`, 'afterEnd', polykit_consistency_output );

	// If the current table has only one editor, already opened, load suggestions for it.
	( $gp.editor.current ) && polykit_do_consistency( $gp.editor.current[ 0 ].querySelector( '.polykit-consistency' ) );
}
...
async function polykit_do_consistency( el ) {
	...
	const current_string = {
		'translated_texts':   [],
		'form_names':         [],
		'alternatives_count': polykit_consistency_get_alternative_count( consistency_page, consistency_alternatives.length ),
	};
	...
	const polykit_consistency_suggestions = polykit_create_element( 'ul', { class: 'polykit-suggestions-list' } );
	...
	for ( let consistency_alternatives_i = 0; consistency_alternatives_i < consistency_alternatives.length; consistency_alternatives_i++ ) {
		...
		if ( translation_forms.length > 1 ) {
			const string_page = await polykit_consistency_get_page( consistency_alternatives[ consistency_alternatives_i ].parentNode.parentNode.nextSibling.querySelectorAll( 'td .meta a' )[ 1 ].href.replace( '?filters', '/?filters' ) );
			...
			const polykit_consistency_item_header = polykit_create_element( 'li', { 'class': 'consistency-header-index' }, `#${consistency_alternatives_i + 1}` );
			polykit_consistency_item_header.append(
				polykit_create_element( 'button', { 'type': 'button', 'class': 'copy-full-alternative', 'data-alternative_id': consistency_alternatives_i }, polykit_t( 'consistency_copy' ) ),
				polykit_create_element( 'span', { 'class': 'consistency-count' }, current_string.alternatives_count[ consistency_alternatives_i ] ),
			);
		...
		polykit_consistency_suggestions.append( polykit_consistency_add_alternative( alternative, current_string ) );
	}
	if ( '1' === $gp_editor_options.can_approve && consistency_alternatives.length > 1 ) {
		const warning = document.createElement( 'div' );
		warning.className = 'gte-warning';
		warning.textContent = polykit_t( 'consistency_warning', consistency_alternatives.length );
		polykit_consistency_suggestions.insertAdjacentElement( 'afterBegin', warning );
	}

	el.append( polykit_consistency_suggestions );
	( translation_forms.length > 1 ) &&	polykit_consistency_format_for_plural( this_panel_content );
...
function polykit_consistency_get_alternative_count( consistency_page, consistency_count ) {
...
function polykit_consistency_add_alternative ( alternative, current_string ) {
	...
	alternative.forms_text.forEach( ( form_text, form_text_i ) => {
		...
		const polykit_consistency_item_div = polykit_create_element( 'div', { 'class': 'translation-suggestion with-tooltip', 'role': 'button', 'aria-pressed': 'false', 'aria-label': polykit_t( 'consistency_copy_translation' ), 'tabindex': '0' } );
		const polykit_consistency_item_translation = polykit_create_element( 'span', { 'class': 'translation-suggestion__translation' } );
		...
		polykit_consistency_item_translation.append( alternative_as_words_fragment, is_this_one );
		const meta_info = ( current_string.form_names.length ) ? `${current_string.form_names[ form_text_i ]}: ` : `${alternative.i + 1}: `;
		const polykit_consistency_item_meta = polykit_create_element( 'span', { 'class': 'translation-suggestion__translation index' }, meta_info );
		const polykit_consistency_item_raw = polykit_create_element( 'span', { 'class': `translation-suggestion__translation-raw consistency_alternative__${alternative.i}_${form_text_i}`, 'aria-hidden': 'true' }, form_text );
		const polykit_consistency_item_button = polykit_create_element( 'button', { 'type': 'button', 'class': 'copy-suggestion' }, polykit_t( 'consistency_copy' ) );
		polykit_consistency_item_translation.prepend( polykit_consistency_item_meta );
		( 0 === current_string.form_names.length ) && polykit_consistency_item_translation.append( polykit_create_element( 'span', { 'class': 'consistency-count' }, current_string.alternatives_count[ alternative.i ] ) );
		polykit_consistency_item_div.append( polykit_consistency_item_translation, polykit_consistency_item_raw, polykit_consistency_item_button );
...
function polykit_consistency_format_for_plural( this_panel_content ) {
	polykit_add_evt_listener( 'click', '.copy-full-alternative', polykit_consistency_copy_full_alternative );
	this_panel_content.querySelectorAll( '.polykit-consistency .copy-suggestion' ).forEach( ( el ) => {
		el.parentNode.removeChild( el );
	} );
	this_panel_content.querySelectorAll( '.polykit-consistency .translation-suggestion' ).forEach( ( el ) => {
		el.classList.remove( 'translation-suggestion' );
	} );
	this_panel_content.querySelectorAll( '.polykit-consistency .with-tooltip' ).forEach( ( el ) => {
...
function polykit_notranslate( current_editor = '.editor' ) {
	const notranslate_header = document.createElement( 'div' );
	notranslate_header.textContent = polykit_t( 'notranslate_title' );
	notranslate_header.append( polykit_create_element( 'button', { 'type': 'button', 'class': 'polykit-notranslate-copy-all' }, polykit_t( 'notranslate_copy_all' ) ) );
	document.querySelectorAll( `${current_editor.replace( 'editor', 'preview' )} .original` ).forEach( ( original_preview ) => {
		...
		const notranslate = polykit_create_element( 'div', { 'class': 'polykit-notranslate' } );
		...
		original_preview_forms[ 0 ].parentNode.querySelectorAll( '.original-text > .notranslate' ).forEach( ( item ) => {
			...
			notranslate_item.setAttribute( 'title', polykit_t( 'notranslate_insert_hint' ) );
	...
	document.querySelectorAll( '.editor .notranslate' ).forEach( ( el ) => { el.setAttribute( 'title', polykit_t( 'notranslate_insert_hint' ) ); } );
	polykit_notranslate_events( current_editor );
}

function polykit_notranslate_events( current_editor ) {
	polykit_add_evt_listener( 'click', `${current_editor} .translation-form-list li button`, ( ev ) => {
	...
	polykit_add_evt_listener( 'focusin', `${current_editor} textarea`, polykit_notranslate_update );
	polykit_add_evt_listener( 'keyup', `${current_editor} textarea`, polykit_notranslate_update );
	polykit_add_evt_listener( 'click', `${current_editor} .polykit-notranslate a, ${current_editor} .notranslate`, ( ev ) => {
		polykit_notranslate_insertText( ev.currentTarget.closest( '.editor-panel__left' ).querySelector( '.textareas.active textarea' ), ev.currentTarget.textContent );
	} );
	polykit_add_evt_listener( 'click', `${current_editor} .polykit-notranslate-copy-all`, ( ev ) => {
		let all_notranslate = '';
		...
		polykit_notranslate_insertText( notranslate_div.closest( '.editor-panel__left' ).querySelector( '.textareas.active textarea' ), all_notranslate );
	} );
...
function polykit_notranslate_insertText( el, text ) {
...
function polykit_notranslate_update( ev ) {
	let textarea_content = ev.currentTarget.value;
	ev.currentTarget.closest( '.editor-panel__left' ).querySelectorAll( '.polykit-notranslate a' ).forEach( ( notranslate_item ) => {
