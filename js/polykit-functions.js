...
function polykit_add_project_links() {
	if ( jQuery( '.gp-content .breadcrumb li' ).length > 3 && jQuery( '.gp-content .breadcrumb li:last-child a' ).length > 0 ) {
		...
		jQuery( '#polykit-title-links' ).append( `<a class="glossary-link" href="https://translate.wordpress.org/locale/${lang}/default" target="_blank" rel="noreferrer noopener">${jQuery( '.gp-content .breadcrumb li:last-child a' ).text()} ${polykit_t( 'projects_suffix' )}</a>` + `<a class="glossary-link" href="https://translate.wordpress.org/stats" target="_blank" rel="noreferrer noopener">${polykit_t( 'translation_global_status' )}</a>` );
		...
		const glossaryLinksSeparator = document.querySelector( '#polykit-title-links .glossary-links .separator' );
...
function polykit_add_glossary_links( glossary_word ) {
	...
	word.wrap( `<a href="https://translate.wordpress.org/consistency?search=${word.text()}&amp;set=${polykit_get_lang_consistency()}%2Fdefault" target="_blank" rel="noreferrer noopener"></a>` );
}
...
function polykit_add_review_button() {
	if ( jQuery( 'body.logged-in' ).length !== 0 && jQuery( '.discussions-table-head' ).length === 0 && ! document.querySelector( '.polykit-review' ) ) {
		...
		if ( filter_toolbar ) {
			...
			filter_toolbar.insertAdjacentElement( 'afterend', review_button );
		}
	}
}
...
function polykit_get_locale_slug( value, type ) {
	...
	for ( const elem in polykit_locales_slugs ) {
		if ( Object.prototype.hasOwnProperty.call( polykit_locales_slugs, elem ) && polykit_locales_slugs[elem] === value ) {
			return elem;
		}
	}
	return '';
...
function polykit_add_scroll_buttons() {
	...
	for ( const regex in locations ) {
		...
		if ( position && acquired ) {
			if ( '' === lang ) {
				jQuery( position ).before( `<span style="float:right;margin-bottom:1em">${polykit_t( 'locale_not_set', 'https://translate.wordpress.org/projects/wp/dev/en-gb/default/#polykit-language-picker' )}</span>` );
				return;
			}
			jQuery( position ).before( `<button style="float:right;margin-bottom:1em" class="polykit-scroll">${polykit_t( 'scroll_to', lang )}</button>` );
...
/**
 * PolyKit controls appended to the filter toolbar row.
 *
 * @param {HTMLElement} filter_toolbars_div
 * @returns {HTMLElement}
 */
function polykit_ensure_toolbar_extensions( filter_toolbars_div ) {
	let group = filter_toolbars_div.querySelector( '.polykit-toolbar-extensions' );
	if ( group ) {
		return group;
	}
	...
	group.className = 'polykit-toolbar-extensions';
	filter_toolbars_div.append( separator, group );
	return group;
}
...
function polykit_locales_selector() {
	...
	const lang = polykit_get_lang();
	const group = polykit_ensure_toolbar_extensions( filter_toolbars_div );
	...
	picker_container.className = `polykit-language-picker-container${( '' === lang || false === lang ) ? ' empty-locale' : ''}`;
...
function polykit_add_official_links_to_filters() {
	...
	if ( '' === polykit_glossary.guide.url && '' === polykit_glossary.handbook_url ) {
		return;
	}
	...
	polykit_guide_link.textContent = '' !== polykit_glossary.guide.title ? polykit_glossary.guide.title : polykit_t( 'default_style_guide' );
	polykit_guide_link.href = '' !== polykit_glossary.guide.url ? polykit_glossary.guide.url : polykit_glossary.handbook_url;
	const group = polykit_ensure_toolbar_extensions( filter_toolbars_div );
		...
		const observer = new MutationObserver( ( mutations ) => {
			mutations.forEach( ( mutation ) => {
				...
				mutation.addedNodes.forEach( ( addedNode ) => {
					...
					if ( row_is_preview ) {
						addedNode.querySelectorAll( '.glossary-word' ).forEach( polykit_add_glossary_links );
					}
					if ( row_is_editor ) {
 ...
 * @returns {Element}
 */
function polykit_create_element( tagName = 'div', attributes = {}, textContent = '' ) {
 ...
 * @returns {void}
 */
function polykit_add_evt_listener( event_name, target_selector, function_to_call ) {
	document.querySelectorAll( target_selector ).forEach( ( el ) => {
 ...
 * @return {void}
 */
function polykit_tag_target_when_source_outside_viewport( sourceElement, targetElement, classElement ) {
...
function polykit_build_sticky_header() {
	...
	const polykit_review = document.querySelector( '.polykit-review' );
	const paging_top = document.querySelector( '.paging' );
	const polykit_notices_container = document.querySelector( '#polykit-notices-container' );
	...
	toggle_sticky_input.addEventListener( 'click', ( e ) => {
		...
		localStorage.setItem( 'polykit_header_is_sticky', ( ( true === polykit_header_is_sticky ) ? 'true' : 'false' ) );
		e.stopPropagation();
	} );
	...
	polykit_review && fragment.appendChild( polykit_review );
	...
	polykit_sticky_header.appendChild( fragment );
...
function polykit_copy_visible_original_string() {
	polykit_copy_to_clipboard( document.querySelector( '.editor[style="display: table-row;"] .original-raw' ).innerHTML );
}
...
function polykit_anonymous() {
	...
	const anonymous_input = polykit_create_element( 'input', { 'type': 'checkbox', 'id': 'polykit-search-anonymous' } );
	const anonymous_label = polykit_create_element( 'label', { 'for': 'polykit-search-anonymous' }, polykit_t( 'anonymous_author' ) );
...
function polykit_pagination() {
	...
	default_pagination.forEach( default_pagination_instance => {
		...
		this_polykit_pagination.addEventListener( 'change', ( ev ) => { window.location = last_page.url.replace( /page=\d+/, `page=${ev.target.value}` ); } );
		default_pagination_instance.insertAdjacentElement( 'beforeend', this_polykit_pagination );
	} );

	document.querySelectorAll( '.polykit-pagination .current-page' ).forEach( el => { el.selected = true; } );
}
