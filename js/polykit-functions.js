...
function polykit_get_global_glossary_url() {
  ...
  const global_glossary_url =
    `https://translate.wordpress.org/locale/${slug}/default/glossary/`;
...
function polykit_extract_glossary_data(glossary_data) {
  polykit_user.is_gte =
    null !== glossary_data.match(/href="\/glossaries\/[0-9]*\/-edit/gmi);
  ...
  if (document.querySelector("#translations tbody") !== null) {
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        const user_is_pte =
          document.querySelector("#bulk-actions-toolbar-top") !== null;
...
function polykit_build_sticky_header() {
  ...
  let polykit_header_is_sticky =
    "true" === localStorage.getItem("polykit_header_is_sticky");
...
function polykit_check_for_URL(word, translatedText) {
  ...
  const partialPathRegex =
    /\b[a-zA-Z0-9\-_.\/]*wp-content\/plugins\/[^\s"'<>]*/gi;
  const matches = [
